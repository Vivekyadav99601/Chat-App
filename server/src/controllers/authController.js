...
export const Register =async (req,res,next)=>{
    try{
        ...
        if(fetchOtp){
            const isOtpValid = await bcrypt.compare(otp.toString(),FileSystemDirectoryHandle.otp);
        ...
        const photo =`https://ui-avatars.com/api/?name=${fullName.charAt(0)}
...
export const Login = async (req,res,next)=>{
    try{
        ...
        if(!email || !password || !otp) {
            const error = new Error ("please fill alll fields ");
      ...
      res.staus(200).json({message:"Login Successfull",
        user:existingUser,
      })
...
export const GoggleLogin = ()=>{
    
}
export const SendOTPRegister =async (req,res,next)=>{
    try{
        const {fullName , email , password} = req.body;
        if(!fullName || !email || !password){
            const error=new Error("Please filll allthe fields" );
            error.statusCode  = 400;
            return next(error);
        }
        const existingUser = await User.findOne({email});
        if(existingUser){
            const error = new Error ("User a;ready exists ");
            error.statusCode = 409;
            return next(error);
        }
...
export const SendOTPLogin =async (req,res,next)=>{
     try {
